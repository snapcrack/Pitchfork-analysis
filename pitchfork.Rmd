...
```{r}
ggplot(aes(score, x = album.number, y = artist, fill = rollmean(score, 3, fill = score, align='right'), group = artist), data = subset(reviews, albums == 13 & artist != 'various artists')) +
  geom_tile()+
  scale_fill_continuous(low = '#0000cc', high = '#ff0000', name = 'Score') +
  scale_x_continuous()+
  xlab('Album Number')+
...
It looks like: maybe??? I like to save regression analyses to the end; it forces you to find phenomena clearly visible if in fact they exist. But we've been unable to do that, let's just do a regression analysis and see what comes up.
...
```{r}
ggplot(aes(y = rollmean(score, 3, fill = score, align='right'), x = album.number, color = artist, group = 2),
